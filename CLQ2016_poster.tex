...
\definecolor{greygher}{RGB}{185, 185, 185}  	% grey 
...
%\setbeamercolor{background canvas}{bg=greybackground}
...
{/home/ctroupin/Presentations/201605_CLQ/Circles/}
}
...
\node[anchor=north east,yshift=0pt,xshift=-.5cm] at (current page.north east) {\includegraphics[height=.07\textheight]{colloquium_logo3}};
\node[anchor=north east,yshift=-5cm,opacity=0.2] at (current page.north east) {\includegraphics[height=8cm]{capeghir_bwr2}};
\end{tikzpicture}}
...
\node[anchor=south west,inner sep=0] (image) at (0,0) {\pgfimage[interpolate=true,width=.6\textwidth]{main_fig2_resize.jpg}};
...
\column{.25\textwidth}
\tiny
\tikz[na] \coordinate (A0); Observations - Modelling - Theory\\
\tikz[na] \coordinate (A2); Air-sea coupling\\
\tikz[na, xshift=-1cm] \coordinate (ABC2);\tikz[na] \coordinate (B2); Surface forcing \\
...
{\fontspec{Quivira.otf}
{\scriptsize \textcolor{bluegher}{\faCalendarCheckO~~~\insertdate}}\\
{\scriptsize \textcolor{bluegher}{{\ComputerMouse}~~~\url{http://modb.oce.ulg.ac.be/colloquium}}}\\
{\scriptsize \textcolor{greygher}{
\house{Quivira.otf}{1F3E0}~~~University of Liège - Place du 20-Août, 7 - 4000 Liège - Belgium\\ 
{\rmfamily \Letter}~~~\href{mailto:oceanphys@ulg.ac.be}{oceanphys@ulg.ac.be} \\
\includegraphics[height=.2cm]{logo_twitter}~~~{\rmfamily@}GHER\_ULg\\
}
}
}
\vspace{.25cm}
...
\putlogo{logo_nasa}\hspace{.5cm}\putlogo{logo_onr}\hspace{.5cm}\putlogo{logo_iugg}%\putlogoB{logo_egu}%\hspace{.5cm}\putlogo{logo_SCOR}\hspace{.5cm}
